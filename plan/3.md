# TASK-3: Configure TypeScript (`tsconfig.json`) with strict settings and appropriate JSX/runtime config.
...
- Dependencies have been installed (from task 2)
- TypeScript configuration needs to be set up with strict settings
- Need to configure JSX and runtime settings appropriately
- tsconfig.json may exist from Vite template but needs customization
...
- [ ] Understand dependencies and prerequisites (tasks 1-2 should be completed)
...
- [ ] Step 1: Create or update tsconfig.json
  ...
  - [ ] Configure JSX: `"jsx": "react-jsx"` (React 17+ JSX transform)
  - [ ] Set `"target": "ES2020"` or higher
  - [ ] Set `"module": "ESNext"` for ESM modules
  - [ ] Set `"moduleResolution": "bundler"` for Vite
  - [ ] Add `"include": ["src"]` and `"exclude": ["node_modules", "dist"]`
  - [ ] Configure path aliases if needed: `"paths": { "@/*": ["./src/*"] }`
- [ ] Step 2: Verify TypeScript configuration
  - [ ] Run `npm run type-check` or `npx tsc --noEmit`
  - [ ] Ensure no type errors in existing files
  - [ ] Verify JSX compilation works correctly
...
- **Execution Timing**: This task should be executed after completing tasks 1-2
- **Dependencies**: 
...
5. **Git Commit and Push**
   ...
   - Commit message follows project conventions: `TASK-3: Configure TypeScript (`tsconfig.json`) with strict settings and appropriate JSX/runtime config.`
