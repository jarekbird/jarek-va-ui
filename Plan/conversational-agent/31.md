...
- Implement a simple text-based interaction path in `AgentConversationDetails.tsx` (which is embedded in `AgentChatPanel` within the Dashboard).
- Store text messages in `elevenlabs:conversation:{conversationId}` via backend (elevenlabs-agent service, NOT cursor-runner).
- Ensure the UI renders mixed voice and text messages consistently within the AgentChatPanel.

## Desired Outcomes

### Primary Outcomes
1. **Text Input Support**: Users can send text messages to agents in addition to voice, providing flexibility in communication within the Dashboard's AgentChatPanel.

2. **Unified Message History**: Voice and text messages appear together in agent conversation history with consistent formatting within the AgentChatPanel.

3. **Message Persistence**: Text messages stored in backend (elevenlabs-agent service) for conversation history and future reference.

4. **Consistent UX**: Text and voice messages rendered consistently with clear indicators of message source within the Dashboard layout.
